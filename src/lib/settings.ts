...
export const DEFAULT_PIN_ON_TOP = true;
...
export async function loadPinOnTop(): Promise<boolean> {
  const stored = await store.get<unknown>(PIN_ON_TOP_KEY);
  if (typeof stored === "boolean") return stored;
  return DEFAULT_PIN_ON_TOP;
}

export async function savePinOnTop(value: boolean): Promise<void> {
  await store.set(PIN_ON_TOP_KEY, value);
  await store.save();
}
